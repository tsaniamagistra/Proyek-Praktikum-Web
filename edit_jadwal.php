<?php
	...
	if (empty($_SESSION['no_rm'])) {
		header("Location:login.php?message=form_janji.php");
	}
...
<body>
	...
	<div style="width:90%; background-color:#063970;" class="py-4 px-2">
		...
		<form method="GET" action="edit_jadwal.php" style="width: 100%;" class="row">
            
            <?php 
		    $query = mysqli_query($connect,"SELECT b.hari, c.id_klinik, c.klinik, d.id_dokter, d.dokter FROM riwayat_pasien AS a INNER JOIN jadwal_dokter 
            AS b ON a.id_jadwal=b.id_jadwal INNER JOIN klinik AS c ON b.id_klinik=c.id_klinik INNER JOIN dokter AS d ON b.id_dokter=d.id_dokter where 
            a.no_pendaftaran='$no_pendaftaran'");
		    $data = mysqli_fetch_array($query);
	        ?>
                    
			<div class="col-4">
			<select class="form-select dropdown-toggle scrollable-select" name="klinik" disabled>
				<option value='<?= $data['id_klinik']?>' selected><?= $data['klinik']?></option>
			</select>
			</div>
			<div class="col-4">
			<select class="form-select col" name="dokter" disabled>
                <option value='<?= $data['id_dokter']?>' selected><?= $data['dokter']?></option>
			</select>
			</div>
			<div class="col-3">
			<select class="form-select col" name="hari">
				...
				<?php for ($i=0; $i < $length; $i++) { ?>
					<option value="<?=$days[$i];?>" <?php if ($days[$i]==$data['hari']): ?>selected<?php endif ?>><?=$days[$i];?></td>
				<?php } ?>
			</select>
			</div>
			<div class="col-1">
